...
start_date = yester.strftime('%Y%m%d')

# date filter options:
#START_FROM = 2  # Integer -- number of days to go back from the current date
START_FROM = start_date  # Start date in YYYYMMDD format
...
if not isinstance(START_FROM, (int, str)):
...
def get_files_and_dirs(url):
    ...
    skip_today_date = today_date.strftime("%Y%m%d")

    for link in soup.find_all('a', href=True):
        link_text = link.text.strip()
        link_href = link['href']

        if link_text in ['.', '..', 'back to photo', 'JOURNAL.JNL', 'ezshare.cfg',
                         'system volume information', skip_today_date] or link_text.startswith('.'):
            continue

        if link_text in 'STR.EDF':
            link_text = 'STR.edf'
...
def check_files(files, url, dir_path):
    for filename, file_url in files:
        ...
        if 'STR.EDF' in filename:
            filename = filename.replace("EDF", "edf")
            print(f'{filename} extension renamed')

        if ('DATALOG' in dir_path and os.path.exists(local_path) and
                OVERWRITE_EXISTING_FILES and SHOW_PROGRESS == 'Verbose'):
            print(f'{filename} replaced')
        if SHOW_PROGRESS and 'DATALOG' not in dir_path:
            print(f'{filename} completed filename')
        elif SHOW_PROGRESS == 'Verbose':
            print(f'{filename} completed (V) filename')
...
def connect_to_wifi(ssid, password=None):
    ...
    please connect manually and press 'C' and then 'Enter' to continue, or any other key and 'Enter' to cancel: """)
        ...
        if result.returncode == 0:
            ...
            response = input(
                "Unable to connect automatically, please connect manually and press 'C' to continue or any other key to cancel: ")
            if response.lower() == 'c':
                return True
            else:
                return False
